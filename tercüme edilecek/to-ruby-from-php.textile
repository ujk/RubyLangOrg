...
"#{foo} is a #{bar}" şeklinde kullanabilirsiniz -- PHP'de olduğu gibi, tek tırnaklı karakter dizilerinde kullanamazsınız.
* Here dökümanları vardır
* PHP 5 gibi rubyde de istisnalar(exception) vardır.
* Oldukça büyük bir standart kütüphanesi vardır.
* Dizeler ve hash veri tipi benzer çalışır, @array()@ yerine @{@ ve @}@ kullanırsanız:
@array('a' => 'b')@  @{'a' => 'b'}@  olur.
